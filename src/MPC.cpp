...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const double weight_cte = 3000.0;  //weight for cross track error
    const double weight_epsi = 3000.0;  //weight for orientation error
    const double weight_v = 1.0;  //weight for velocity
    const double weight_delta = 5.0;  //weight for steering angle
    const double weight_a = 5.0;  //weight for acceleration
    const double weight_delta_diff = 200.0;  //weight for delta differentiate
